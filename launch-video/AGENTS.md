...
**Always invoke the relevant skill before writing or modifying compositions.** Skills encode framework-specific patterns (e.g., `window.__hfAnime` registration, `data-*` attribute semantics, shader-compatible CSS rules) that are NOT in generic web docs. Skipping them produces broken compositions.
...
pnpx hyperframes lint --verbose  # include info-level findings
pnpx hyperframes lint --json     # machine-readable output for CI
pnpx hyperframes docs <topic> # reference docs in terminal
```
...
```bash
pnpx hyperframes docs <topic>
```

Topics: `data-attributes`, `compositions`, `rendering`, `examples`, `troubleshooting`
...
3. Keep a no-op `window.__timelines["composition-id"]` compatibility entry, then register Anime.js animations on `window.__hfAnime`:
   ```js
