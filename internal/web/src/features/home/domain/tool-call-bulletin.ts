...
export function parseToolCallBulletinsFromMetadata(
  metadataJson: string
): ToolCallBulletinItem[] {
  ...
  const value = metadata.toolCallBulletins;
  if (!Array.isArray(value)) {
    return [];
  }

  const items: ToolCallBulletinItem[] = [];
  ...
  for (const line of value) {
    ...
    seen.add(trimmed);
    const parsed = parseToolCallBulletin(trimmed);
    if (!parsed) {
      continue;
    }
    items.push({
      ...
      text: formatToolCallBulletinText(parsed),
    });
    if (items.length >= 3) {
      break;
    }
