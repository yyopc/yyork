    ...
    >
      ...
      <div className="yyork-file-preview-body">
        {!props.selectedPath ? (
          ...
          <CanvasPlaceholder centered title="Loading file" detail={props.selectedPath} />
        ) : fileIsError ? (
          <CanvasPlaceholder
            centered
  ...
  return (
    <div
      className={`flex min-h-0 max-w-full flex-col gap-2 p-3 text-sm leading-5${
        props.centered ? ' h-full flex-1 items-center justify-center text-center' : ' h-full'
      }`}
    >
      {props.icon ? (
        <>
      ...
      <p className={props.centered ? 'text-muted-foreground' : 'break-all text-muted-foreground'}>
        {props.detail}
      </p>
    </div>
  );
